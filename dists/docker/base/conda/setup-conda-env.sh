...
if [[ $add_tf_env -eq 1 ]]; then
  ...
  echo "export LD_LIBRARY_PATH=$CONAN_ROOT/envs/$env_name/lib/python$python_short_version/site-packages/nvidia/cudnn/lib:/usr/local/cuda-11.4:/nix/var/nix/profiles/cuda11_4/lib:\$LD_LIBRARY_PATH" >>$CONAN_ROOT/envs/$env_name/etc/conda/activate.d/env_vars.sh
fi
